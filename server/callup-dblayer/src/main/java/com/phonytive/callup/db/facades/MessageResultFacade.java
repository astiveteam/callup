...
import com.phonytive.callup.db.entities.MessageResult;
 ...
 * @since 1.0.0
 */
public class MessageResultFacade {

    @PersistenceContext
    private EntityManager em = new EntityManager();

    public void create(MessageResult messageResult) {
        em.persist(messageResult);
    }

    public void edit(MessageResult messageResult) {
        em.merge(messageResult);
    }

    public void remove(MessageResult messageResult) {
        em.remove(messageResult);
    }

    public Subscriber find(ObjectId id) {
        return (Subscriber) em.findOne(MessageResult.class, id);
    }

    public List<Subscriber> findAll() {
        return em.find(MessageResult.class);
    }
}
